...
import clinicTools from "@/assets/clinic-tools.jpg";
import clinicReception from "@/assets/clinic-reception.jpg";

const heroSlides = [
  { src: heroImage, alt: "Cabinet dentaire moderne à Kasserine" },
  { src: clinicChair, alt: "Salle de soins équipée — Cabinet Dr. Alouani" },
  { src: clinicTools, alt: "Instruments stérilisés et de dernière génération" },
  { src: clinicReception, alt: "Accueil chaleureux du cabinet" },
];
  ...
  return (
    <SiteLayout>
      ...
      <section className="relative overflow-hidden">
        <div className="mx-auto grid max-w-6xl items-center gap-10 px-4 py-16 md:grid-cols-2 md:py-24">
          <div className="space-y-6">
            <div className="inline-flex items-center gap-2 rounded-full border border-border bg-primary/30 px-3 py-1 text-xs text-muted-foreground">
              <span className="h-1.5 w-1.5 rounded-full bg-accent" />
              Cabinet dentaire à Kasserine
            </div>
            <h1 className="text-4xl leading-[1.05] md:text-6xl">
            ...
            >
              <CarouselContent>
                {heroSlides.map((slide, i) => (
                  <CarouselItem key={i}>
                    <div className="tilt-image overflow-hidden rounded-3xl shadow-[var(--shadow-card)]">
                      <img
                        src={slide.src}
                        alt={slide.alt}
                        width={1536}
              ...
              <CarouselPrevious className="left-3 top-1/2 -translate-y-1/2 border-0 bg-background/80 backdrop-blur-md shadow-lg hover:bg-background" />
              <CarouselNext className="right-3 top-1/2 -translate-y-1/2 border-0 bg-background/80 backdrop-blur-md shadow-lg hover:bg-background" />
                ...
                {Array.from({ length: count }).map((_, i) => (
                  <button
                    ...
                    className={`h-2 rounded-full transition-all ${
                      i === current ? "w-8 bg-primary" : "w-2 bg-background/70"
                    }`}
                  />
                ))}
              </div>
      ...
      <section className="mx-auto max-w-6xl px-4 py-20">
        ...
        <div className="grid gap-5 perspective-1200 md:grid-cols-2 lg:grid-cols-4">
          {services.slice(0, 8).map((s) => (
            <div
              key={s.title}
              className="card-3d group rounded-2xl border border-border bg-card p-6"
            >
      ...
      <section className="px-4 pb-20">
        ...
        >
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-3">
            <Button asChild size="lg" variant="destructive" className="btn-3d">
              <Link to="/contact">Prendre rendez-vous</Link>
            </Button>
            <Button asChild size="lg" variant="secondary" className="btn-3d">
              <a href="tel:+21677000000">Appeler maintenant</a>
            </Button>
          </div>
        </div>
